...
- **vibesignal**（本仓库，fork 自 yzhao062/vibesignal）：AI 编码客户端的桌面状态面板。
  定制内容见 `CUSTOMIZATIONS.md`（深色主题、右上角停靠、Tk 9 无边框修复、Dock 图标、订阅用量 footer）。
...
node --version && npm --version   # ccusage 需要；没有则 brew install node
claude --version        # Claude Code 已安装且已登录（用量 footer 依赖其钥匙串 OAuth token）
security find-generic-password -s "Claude Code-credentials" >/dev/null && echo "钥匙串凭据 OK"
```

Intel Mac 注意：Homebrew 前缀是 `/usr/local` 而非 `/opt/homebrew`，`vibesignal/widget.py` 里的
`_CCUSAGE = "/opt/homebrew/bin/ccusage"` 需要同步改。
...
把 4 条 vibesignal 钩子设为信任。验证：`grep -c "hooks.json:" ~/.codex/config.toml` 的数字应增加 4。
之后跑一轮真实对话，`vibesignal status` 出现 `codex/…` 行。
...
```bash
npm install -g ccusage && ccusage --version
```

- 官方订阅百分比：widget 每 5 分钟从钥匙串读 Claude Code 的 OAuth token 调
  `api.anthropic.com/api/oauth/usage`（非公开接口）。**[可能需要人类]** 首次运行如弹出
  钥匙串授权框，选"始终允许"。接口失效时自动降级为 ccusage 本地统计，不影响面板。
- 验证：`env -i HOME="$HOME" PATH=/usr/bin:/bin \
  ...
  应输出形如 `5h 24% (4h16m) · wk 11% (6d16h) · today $93.7`。
