...
> 目标：把本仓库的定制版 vibesignal 及全部周边（hooks、图标、Codex / Claude Code 剩余额度 footer、Cowork 桥接）在一台新 Mac 上恢复到与源机器一致的状态。
> 原则：每步都有验证命令，验证不过不要进入下一步。标注 **[需要人类]** 的步骤 AI 无法代劳，必须明确请用户操作。

## 0. 这套东西是什么

- **vibesignal**（本仓库，fork 自 yzhao062/vibesignal）：AI 编码客户端的桌面状态面板。
  定制内容见 `CUSTOMIZATIONS.md`（高对比深色主题、右上角停靠、Tk 9 无边框修复、Dock 图标、可选 provider 剩余额度 footer）。
...
```bash
~/.local/bin/vibesignal install-launcher --usage-provider auto
~/.local/bin/vibesignal install-autostart --usage-provider auto      # widget 立即启动并登录自启
```
...
面板出现在**主屏右上角**（多显示器时在"键盘焦点所在屏"的右上角），文字为高对比白色粗体，底部剩余额度状态栏清晰可读。

状态栏 provider 选择：

```bash
# Codex 额度
~/.local/bin/vibesignal install-launcher --usage-provider codex
~/.local/bin/vibesignal install-autostart --usage-provider codex

# Claude Code 额度
~/.local/bin/vibesignal install-launcher --usage-provider claude
~/.local/bin/vibesignal install-autostart --usage-provider claude

# 自动选择：先 Codex，失败再 Claude Code
~/.local/bin/vibesignal install-autostart --usage-provider auto
```

`off` 会关闭底部剩余额度状态栏。切换 provider 后重跑 `install-autostart` 并重启 widget。
...
- `--usage-provider codex`：widget 每 5 分钟读取 `~/.codex/auth.json` 的 ChatGPT access token，优先调
  `https://chatgpt.com/backend-api/codex/usage`；接口不可用时读取本机
  `~/.codex/sessions/**/*.jsonl` 最新 `rate_limits`。
- `--usage-provider claude`：widget 读取 macOS Keychain 里的 Claude Code OAuth token，调
  Anthropic OAuth usage 接口。
- footer 显示 5 小时窗口和 7 天窗口的剩余比例与 reset 倒计时，例如
  `5h 57% (4h40m) · 7d 64% (5d16h)`。没有可用额度数据时 footer 留空，不影响状态面板。
- 验证：`env -i HOME="$HOME" PATH=/usr/bin:/bin \
  ~/.local/share/uv/tools/vibesignal/bin/python -c "from vibesignal.widget import _fetch_usage; print(_fetch_usage('auto'))"`
  应输出形如 `5h 57% (4h40m) · 7d 64% (5d16h)`。
